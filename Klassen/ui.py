...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, project_path: str):
        ...
        self.author_input = QtWidgets.QLineEdit(); self.author_input.setPlaceholderText("Ihr Name")
        self.cover_graphic_button = QtWidgets.QPushButton("Deckblatt-Grafik...")
        self.tree_widget = QtWidgets.QTreeWidget(); self.tree_widget.setColumnCount(6)
        self.tree_widget.setHeaderLabels(["Benennung", "Pos.", "Menge", "Bestellnummer", "Information", "Grafik"])
        self.tree_widget.setColumnWidth(0, 350); self.tree_widget.setColumnWidth(1, 50); self.tree_widget.setColumnWidth(2, 80); self.tree_widget.setColumnWidth(3, 180); self.tree_widget.setColumnWidth(4, 250); self.tree_widget.setColumnWidth(5, 100)
        ...
        if sys.platform == "win32": self.update_fields_checkbox.setChecked(True)
        else: self.update_fields_checkbox.setVisible(False); self.update_fields_checkbox.setChecked(False)
        self.save_button = QtWidgets.QPushButton("Auswahl speichern")
        self.load_button = QtWidgets.QPushButton("Auswahl laden")
        
        # NEU: Button für Info/Copyright
        self.info_button = QtWidgets.QPushButton("Info / Copyright")

        # --- Layout ---
        top_layout = QtWidgets.QHBoxLayout()
        top_layout.addWidget(QtWidgets.QLabel("Hauptbaugruppe:")); top_layout.addWidget(self.assembly_selector, 2)
        ...
        bottom_layout.addWidget(self.info_button) # Hinzugefügt
        bottom_layout.addWidget(self.load_button); bottom_layout.addWidget(self.save_button)
        bottom_layout.addStretch()
        bottom_layout.addWidget(self.update_fields_checkbox)
        bottom_layout.addWidget(self.generate_button)
        
        main_layout = QtWidgets.QVBoxLayout()
        main_layout.addLayout(top_layout); main_layout.addWidget(self.tree_widget); main_layout.addLayout(bottom_layout)
        central_widget = QtWidgets.QWidget(); central_widget.setLayout(main_layout); self.setCentralWidget(central_widget)
        ...
        self.info_button.clicked.connect(self._show_info_dialog) # Hinzugefügt
    ...
    def _load_project_data(self):
        boms_path = os.path.join(self.project_path, "stücklisten")
        if not os.path.isdir(boms_path): return
        processor = BomProcessor(folder_path=boms_path)
    ...
    def _on_save_selection_clicked(self, is_initial_save=False):
        ...
        if is_initial_save: save_path = os.path.join(self.project_path, f"projekt_{os.path.basename(self.project_path)}.json")
        else: save_path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Auswahl speichern", self.project_path, "JSON-Dateien (*.json)")
        if save_path:
            with open(save_path, 'w', encoding='utf-8') as f: json.dump(save_data, f, indent=4, ensure_ascii=False)
            if not is_initial_save: QtWidgets.QMessageBox.information(self, "Erfolg", "Die aktuelle Auswahl wurde gespeichert.")
    ...
    def _on_assembly_selected(self, bom_znr: str):
        if bom_znr in self.all_boms: self._populate_tree(self.all_boms[bom_znr])
    def _populate_tree(self, main_assembly):
        self.tree_widget.blockSignals(True); self.tree_widget.clear(); self.item_lookup.clear()
        ...
        root_item.setFont(0, self._get_bold_font()); root_item.setFlags(root_item.flags() | QtCore.Qt.ItemFlag.ItemIsUserCheckable); root_item.setCheckState(0, QtCore.Qt.CheckState.Checked)
        ...
        assign_button = QtWidgets.QPushButton("Zuordnen..."); assign_button.clicked.connect(partial(self._on_assign_graphic_clicked, root_item)); self.tree_widget.setItemWidget(root_item, 5, assign_button)
        self._add_children_recursively(root_item, main_assembly)
        self.tree_widget.expandAll(); self.tree_widget.blockSignals(False)
    def _add_children_recursively(self, parent_item, bom_obj):
        for position in bom_obj.positionen:
            child_item = QtWidgets.QTreeWidgetItem(parent_item)
            child_item.setText(0, str(position.get('Benennung', ''))); child_item.setText(1, f"{position.get('POS', ''):g}"); child_item.setText(2, str(position.get('Menge', ''))); child_item.setText(3, str(position.get('Bestellnummer_Kunde'))); child_item.setText(4, str(position.get('Information', '')))
            child_item.setFlags(child_item.flags() | QtCore.Qt.ItemFlag.ItemIsUserCheckable); child_item.setCheckState(0, QtCore.Qt.CheckState.Checked)
            parent_znr = str(bom_obj.zeichnungsnummer); pos_nr = str(position.get('POS', '')); unique_id = f"{parent_znr}_{pos_nr}"
            position_data = position.copy(); position_data['is_assembly'] = 'sub_assembly' in position
            ...
            if position_data['is_assembly']:
                child_item.setFont(0, self._get_bold_font()); assign_button = QtWidgets.QPushButton("Zuordnen..."); assign_button.clicked.connect(partial(self._on_assign_graphic_clicked, child_item)); self.tree_widget.setItemWidget(child_item, 5, assign_button)
    ...
    def _on_assign_cover_graphic_clicked(self):
        ...
        if file_path:
            grafik_folder = os.path.join(self.project_path, "Grafik"); os.makedirs(grafik_folder, exist_ok=True)
            _, file_extension = os.path.splitext(file_path); destination_path = os.path.join(grafik_folder, f"EL{file_extension}")
            try: shutil.copy(file_path, destination_path); QtWidgets.QMessageBox.information(self, "Erfolg", "Deckblatt-Grafik wurde erfolgreich gespeichert.")
            except Exception as e: QtWidgets.QMessageBox.critical(self, "Fehler beim Kopieren", f"Die Grafik konnte nicht gespeichert werden:\n{e}")
    def _on_assign_graphic_clicked(self, item):
        ...
        if not zeichnung_nr: QtWidgets.QMessageBox.warning(self, "Fehler", "Für diese Position wurde keine Zeichnungsnummer gefunden."); return
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Grafik auswählen", self.project_path, "Bilder (*.png *.jpg *.jpeg)")
        if file_path:
            grafik_folder = os.path.join(self.project_path, "Grafik"); os.makedirs(grafik_folder, exist_ok=True)
            _, file_extension = os.path.splitext(file_path); destination_path = os.path.join(grafik_folder, f"{zeichnung_nr}{file_extension}")
            try: shutil.copy(file_path, destination_path); QtWidgets.QMessageBox.information(self, "Erfolg", f"Grafik wurde erfolgreich für {zeichnung_nr} gespeichert.")
            except Exception as e: QtWidgets.QMessageBox.critical(self, "Fehler beim Kopieren", f"Die Grafik konnte nicht gespeichert werden:\n{e}")
    def _on_generate_button_clicked(self):
        ...
        main_bom_znr = self.assembly_selector.currentText(); main_bom_obj = self.all_boms.get(main_bom_znr)
        ...
        generator = DocxGenerator(data=hierarchical_data, main_bom=main_bom_obj, author_name=author_name, template_path=template_path, output_path=output_path, auto_update_fields=auto_update, project_path=self.project_path)
        success = generator.run()
        if success:
            msg_box = QtWidgets.QMessageBox(); msg_box.setIcon(QtWidgets.QMessageBox.Icon.Information); msg_box.setText("Erfolg!")
            ...
            if msg_box.clickedButton() == button_open: 
                ...
                except AttributeError: print(f"Datei kann nicht automatisch geöffnet werden. Pfad: {output_path}")
    ...
    def load_data(self, all_boms: dict):
        self.all_boms = all_boms; self.assembly_selector.blockSignals(True); self.assembly_selector.clear()
        if not self.all_boms: self.assembly_selector.addItem("Keine Stücklisten gefunden"); self.assembly_selector.setEnabled(False); self.tree_widget.clear()
        else: self.assembly_selector.addItems(sorted(self.all_boms.keys())); self.assembly_selector.setEnabled(True)
        self.assembly_selector.blockSignals(False)
        if self.assembly_selector.count() > 0: self._on_assembly_selected(self.assembly_selector.currentText())
    def _collect_hierarchical_data(self, parent_item):
        ...
        parent_data.pop('sub_assembly', None); parent_data['children'] = []
        for i in range(parent_item.childCount()):
            child_item = parent_item.child(i); child_data = self._collect_hierarchical_data(child_item)
            if child_data: parent_data['children'].append(child_data)
        return parent_data
    def _handle_item_changed(self, item, column):
        if column == 0: self.tree_widget.blockSignals(True); self._set_children_checkstate(item, item.checkState(0)); self.tree_widget.blockSignals(False)
    def _set_children_checkstate(self, parent_item, state):
        for i in range(parent_item.childCount()):
            child = parent_item.child(i); child.setCheckState(0, state)
            if child.childCount() > 0: self._set_children_checkstate(child, state)
    def _get_bold_font(self): font = QtGui.QFont(); font.setBold(True); return font
